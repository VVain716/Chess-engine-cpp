...
def load_config(config_file: str) -> Configuration:
    ...
    if not os.path.exists(config_file):
        default_candidate = os.path.join(os.path.dirname(config_file) or ".", "config.yml.default")
        if os.path.exists(default_candidate):
            logger.info(f"{config_file} not found; using template from {default_candidate}")
            config_file = default_candidate
    ...
    for env_key in ["LICHESS_BOT_TOKEN", "LICHESS_TOKEN", "TOKEN"]:
        if os.environ.get(env_key):
            CONFIG["token"] = os.environ[env_key]
            break
